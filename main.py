...
if page == "レシート撮影📷":
  ...
  # AI解析の保持用ステート
  if "analyzed_receipt_items" not in st.session_state:
    st.session_state.analyzed_receipt_items = None
  if "analyzed_image_file" not in st.session_state:
    st.session_state.analyzed_image_file = None
  ...
  if img_file is not None:
    ...
    st.image(
        preview_img, caption="選択されたレシート", use_container_width=True
    )

    # まだ解析していない場合、または別の画像が選ばれた場合
    if (
        st.session_state.analyzed_receipt_items is None
        or st.session_state.analyzed_image_file != img_file
    ):
        ...
        with st.spinner("AIがレシートを解析しています...⏳"):
          try:
            ...
            prompt = """
                    ...
                    小計、合計、値引き、レジ袋、お釣りなどの行も含めて構いませんが、純粋な商品明細を正確に読み取ってください。
                    出力は必ず以下のJSON形式の配列のみで返してください。余計な文章は含めないでください。
                    [
                      {"name": "商品名A", "price": 300},
                      {"name": "商品名B", "price": 150}
                    ]
            ...
            st.session_state.analyzed_receipt_items = items
            st.session_state.analyzed_image_file = img_file
            st.success("解析が完了しました！下のリストから必要なものを選んでください。")
            st.rerun()
          except Exception as e:
            st.error(f"AI解析エラーが発生しました: {e}")

    # 解析結果が存在する場合：取捨選択・修正UIの表示
    if st.session_state.analyzed_receipt_items is not None:
      st.write("---")
      st.subheader("📝 読み取り結果の確認・取捨選択")
      st.write(
          "家計簿に残したい商品にチェックを入れ、必要があれば名前や価格を直してください。"
      )

      # 登録先の場所（ストア名など）を取得用のcategoriesから選択または入力できるようにする
      cats = get_data("categories")
      df_cats = (
          pd.DataFrame(cats) if cats else pd.DataFrame(columns=["place", "item"])
      )
      places = (
          sorted(df_cats["place"].unique().tolist())
          if not df_cats.empty
          else []
      )

      selected_place = st.selectbox(
          "どこで購入した？（場所）", [""] + places, index=0
      )
      custom_place = st.text_input("または新しい場所を直接入力", value="")
      final_place = (
          custom_place.strip()
          if custom_place.strip()
          else (selected_place if selected_place else "その他")
      )

      edited_items = []
      total_checked_price = 0

      for i, item in enumerate(st.session_state.analyzed_receipt_items):
        col_chk, col_name, col_price = st.columns([1, 4, 3])
        with col_chk:
          is_checked = st.checkbox("保存", value=True, key=f"rec_chk_{i}")
        with col_name:
          name_val = st.text_input(
              "品名", value=item.get("name", ""), key=f"rec_name_{i}"
          )
        with col_price:
        ...
        if is_checked:
          edited_items.append({"name": name_val, "price": price_val})
          total_checked_price += price_val

      st.markdown(
          f"### 💡 選択された項目の合計金額: **{total_checked_price:,} 円**"
      )

      col_save_btn, col_cancel_btn = st.columns(2)
      with col_save_btn:
        if st.button(
            "💾 選択した項目を家計簿に一括登録 & 画像保存",
            use_container_width=True,
        ):
          if not final_place:
            st.error("購入した場所を指定または入力してください！")
          elif not edited_items:
            st.error("保存する項目が1つも選択されていません！")
          else:
            with st.spinner(
                "画像を保存し、選択項目を家計簿に登録中...⏳"
            ):
              # 1. レシート画像をFirebase Storageに保存
              ...
              # 2. 選択された各商品を expenses コレクションに自動登録
              for ed_item in edited_items:
                db.collection("expenses").add({
                    "person": current_user,
                    "place": final_place,
                    "item": ed_item["name"],
                    "amount": int(ed_item["price"]),
                    "is_reimburse": False,
                    "timestamp": firestore.SERVER_TIMESTAMP,
                    "is_archived": False,
                })
      ...
      with col_cancel_btn:
        if st.button("やり直す（リセット）", use_container_width=True):
          st.session_state.analyzed_receipt_items = None
          st.session_state.analyzed_image_file = None
          st.session_state.uploader_key += 1
          st.rerun()
  ...
  confirm_all_del = st.checkbox(
      f"🚙本当に【{current_user}】のレシートを全削除する？栗"
  )
  if st.button(
      f"🚨 {current_user}のを全削除する",
      use_container_width=True,
      disabled=not confirm_all_del,
  ):
    with st.spinner(f"{current_user}のデータを一括削除中...⏳"):
      # ログイン中のユーザーのレシートのみを取得
      my_receipts = (
          db.collection("receipt_images")
  ...
  if receipts:
    ...
    # タイムゾーン変換処理
    timestamps = [
        d.get("timestamp") if isinstance(d, dict) else d
        for d in df_receipts["timestamp"]
    ]
    ...
    df_receipts["timestamp"] = df_receipts["timestamp"].dt.tz_convert(
        "Asia/Tokyo"
    )
    df_receipts["日時"] = (
        df_receipts["timestamp"]
...
# --- リスト管理 ---
  ...
  if cats:
    ...
    st.dataframe(
        df_cats[["place", "item"]].rename(
            columns={"place": "場所", "item": "品"}
        ),
        use_container_width=True,
        hide_index=True,
    )
    with st.expander("🏺リストから削除🐸"):
      options = {
          f"{r['place']} - {r['item']}": r["id"] for _, r in df_cats.iterrows()
      }
...
# --- ToDoリスト ---
  ...
  if todos:
    ...
    df_todos["due_date_dt"] = pd.to_datetime(
        df_todos["due_date"], errors="coerce"
    )
    ...
    for _, todo in df_todos.iterrows():
      ...
      due_dt = (
          todo["due_date_dt"].date()
          if pd.notnull(todo["due_date_dt"])
          else None
      )
      ...
      with c_del:
        if st.button(
            "完了✨", key=f"todo_del_{todo_id}", use_container_width=True
        ):
  ...
  with st.expander("➕ 新しいToDoを追加する", expanded=False):
    with st.form("add_todo_form"):
      ...
      todo_content = c_input.text_input(
          "やる事の内容", placeholder="例：電気代の支払い、ゴミ出し など"
      )
      due_date = c_date.date_input(
          "期限", value=date.today() + timedelta(days=1)
      )
      submit_todo = st.form_submit_button(
          "ToDoを追加⚡", use_container_width=True
      )
...
# --- メモ帳ページ ---
  ...
  with st.form("simple_memo_form"):
    ...
    save_btn = st.form_submit_button(
        "🦒最新状態に更新する🐪", use_container_width=True
    )
...
# --- 月別集計・リreset ---
  ...
  if all_expenses:
    ...
    for month in sorted(df_all["month"].dropna().unique(), reverse=True):
      ...
      with st.expander(f"{month} (合計: {df_m['amount'].sum():,}円)"):
        st.dataframe(
            df_m[["person", "place", "item", "amount"]].rename(
                columns={
                    "person": "誰",
                    "place": "場所",
                    "item": "品",
  ...
  st.write(
      f"大地: {'👼' if status.get('daichi') else '💀'} | 日向子:"
      f" {'👼' if status.get('hinako') else '💀'}"
  )
  ...
  if status.get("daichi") and status.get("hinako"):
    if st.button("精算完了(アーカイブ)"):
      for doc in (
          db.collection("expenses").where("is_archived", "==", False).stream()
      ):
...
# --- 管理者設定(削除ページ) ---
  ...
  st.write(
      f"現在の同意状況: 大地 {'👼' if status.get('daichi') else '💀'} / 日向子"
      f" {'👼' if status.get('hinako') else '💀'}"
  )

  user_key = "daichi" if current_user == "大地" else "hinako"
  if st.button(
      f"自分の同意状態を切り替える (現在: {'👼' if status.get(user_key) else '💀'})"
  ):
    status[user_key] = not status.get(user_key, False)
    consent_ref.set(status)
    st.rerun()
  ...
  if confirm and status.get("daichi") and status.get("hinako"):
    ...
    if all_expenses:
      ...
      target_month = st.selectbox(
          "削除したい年月を選択", sorted(df_all["month"].dropna().unique())
      )
    ...
    if st.button("【アーカイブ済み期間】のデータを完全に削除する"):
      for doc in (
          db.collection("expenses").where("is_archived", "==", True).stream()
      ):
        doc.reference.delete()
      consent_ref.set({"daichi": False, "hinako": False})
      st.cache_data.clear()
...
# --- 家計簿入力 (台帳入力) ---
  ...
  if "place_sel" not in st.session_state:
    st.session_state.place_sel = ""

  with st.expander("🐔記録する", expanded=True):
    c1, c2 = st.columns(2)
    st.session_state.place_sel = c1.selectbox(
        "場所選択",
        [""] + sorted(df_cats["place"].unique().tolist()),
        label_visibility="collapsed",
        placeholder="場所選択🐎",
    ...
    available_items = (
        df_cats[df_cats["place"] == st.session_state.place_sel]["item"]
        .unique()
        .tolist()
        if st.session_state.place_sel
        else []
    )
    sel_i = c2.selectbox(
        "品目選択",
    ...
    c3, c4 = st.columns(2)
    txt_p = c3.text_input(
        "場所(直接入力)", label_visibility="collapsed", placeholder="場所(直入力)🥄"
    )
    txt_i = c4.text_input(
        "品目(直接入力)", label_visibility="collapsed", placeholder="品(直入力)🥬"
    )

    c5, c6 = st.columns([2, 1])
    amount = c5.number_input(
        "金額(円)",
        value=None,
        min_value=0,
        step=1,
  ...
  if expenses:
    ...
    st.subheader("🐢 精算結果")
    if bal > 0:
      st.warning(f"💗 日向子から大地へ {int(bal):,} 円")
    elif bal < 0:
      st.warning(f"🐢 大地から日向子へ {int(abs(bal)):,} 円")
    else:
      st.success("貸し借りなし！")
    ...
    def show(c, u):
      with c:
        ...
        udf["日時"] = (
            udf["timestamp"].dt.strftime("%-m/%-d %H:%M").fillna("-")
        )
        st.dataframe(
            udf[["日時", "place", "item", "amount", "is_reimburse"]].rename(
                columns={
                    "place": "場所",
                    "item": "品",
                    "amount": "￥",
        ...
        if u == current_user:
          with st.expander("🍅 削除"):
            opts = {
                f"{r['日時']} {r['place']} {r['item']} {r['amount']}円": r["id"]
